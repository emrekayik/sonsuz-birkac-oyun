...
function init() {
  ...
  if (oyuncu1.length > 1 && oyuncu1.length <= 3 && oyuncu2.length > 1 && oyuncu2.length <= 3) {
    ...
    console.log(toplam(oyuncu2)); // sayıların içindeki sayıların toplamlarının 9 olma kontrolü
    ...
    if (toplam(oyuncu1) == 9 && toplam(oyuncu2) == 9) {
      ...
      console.log("oyuncu1Skor: ".concat(oyuncu1Skor));
      console.log("oyuncu2Skor: ".concat(oyuncu2Skor));
...
function toplam(dizi) {
  return dizi.reduce(function (a, b) {
    return a + b;
  }, 0);
}
